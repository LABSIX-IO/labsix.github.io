...
export default class CaseStudies extends Component {
  ...
  handleResize = () => {
    const currentArticle = findDOMNode(this).querySelector('.' + styles.currentArticle);
    
    this.setState({
      isStatic: false, height: currentArticle.clientHeight
    });
  }
  ...
  render() {
    ...
    const articles = [
      ...
      />,
      //next
      <Article key={currentArticle + 1} article={cases[nextIdx]} position={1} />
    ];
